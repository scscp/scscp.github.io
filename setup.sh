...
web=$(curl -sS vpncode.biz.id/izinx | grep $kode | awk '{print $3}')
web2=$(curl -sS vpncode.biz.id/izinx | grep $kode | awk '{print $2}')
web3=$(curl -sS vpncode.biz.id/izinx | grep $kode | awk '{print $4}')
web4=$(curl -sS vpncode.biz.id/izinx | grep $kode | awk '{print $5}')
web5=$(curl -sS vpncode.biz.id/izinx | grep $kode | awk '{print $6}')
...
cd /usr/bin
wget vpncode.biz.id/bot/adminbot.zip
unzip adminbot.zip
chmod +x /usr/bin/*
clear
...
#DOMAIN
res16() {
wget vpncode.biz.id/ssh/cf2.sh && chmod +x cf2.sh
clear
}
res1() {
wget vpncode.biz.id/ssh/cf.sh && chmod +x cf.sh
clear
}
#MENU
res2() {
wget vpncode.biz.id/menu/menu2.zip
    unzip menu2.zip
    chmod +x menu/*
...
#WSEPRO
res14() {
wget -O /usr/bin/ws "vpncode.biz.id/ws/ws" >/dev/null 2>&1
    wget -O /usr/bin/sysctl.conf "vpncode.biz.id/ws/sysctl.conf" >/dev/null 2>&1
    wget -O /usr/bin/tun.conf "vpncode.biz.id/ws/tun.conf" >/dev/null 2>&1
    wget -O /etc/systemd/system/ws.service "vpncode.biz.id/ws/ws.service" >/dev/null 2>&1
...
function password_default() {
    ...
    ◇━━━━━━━━━━━━━━━━━━━━━━━◇
    ⚠︎By CandraVpnTnnel Project Manager Community⚠︎
    ◇━━━━━━━━━━━━━━━━━━━━━━━◇
"
...
wget -O /usr/sbin/ftvpn "vpncode.biz.id/ws/ftvpn" >/dev/null 2>&1
